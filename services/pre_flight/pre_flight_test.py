...
class PreFlightTest(unittest.TestCase):
    ...
    def test_get_new_mission(self):
        ...
        response = self.app.get('/new-mission', headers=headers)
    ...
    def test_save_mission(self):
        ...
        response = self.app.post('/save-mission', data=data_to_send, headers=headers)

        self.assertEqual(response.status_code, 200)
        response_data = json.loads(response.data)
        self.assertTrue(response_data['result'][0])

    def test_save_mission_unauthorized(self):

        data_to_send = MultiDict([('Pilot-name', 'test'), ('Co-Pilot-name', 'test'), ('Flight-height', '100'), ('Flight-mode', 'Open'), ('Comment', 'test comment')])
        response = self.app.post('/save-mission', data=data_to_send)
